...
export interface GoogleAccountTokenType {
    ...
    addGoogleToken: Mock<(googleTokenObject: GoogleAccountToken) => Promise<void>>;
    removeGoogleToken: Mock<() => void>;
    isTokenExpired: Mock<() => boolean>;
    error: string | null;
}

export const MockedGoogleTokenProvider: React.FC<{ children: React.ReactNode } & GoogleAccountTokenType> = ({
    children,
    ...providerProps
}) => {
...
export const googleProviderPropsInit = (): GoogleAccountTokenType => {
    const providerProps = {
        ...
        isTokenExpired: vi.fn(() => {
            if (!providerProps.googleToken) return true;
