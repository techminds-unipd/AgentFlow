...
describe("ExecuteWorkflowService", () => {
  ...
  test("Should return the execution result from the AI agent", async () => {
    fetchSpy.mockResolvedValue({
      status: 201,
      text: () => Promise.resolve("Execution result")
    } as Response);
    ...
    expect(fetchSpy).toBeCalledWith(`${API_BASE_URL}/workflow/execute`, {
      ...
      body: JSON.stringify({ workflow: workflow, googleToken: { token: googleToken.token, refreshToken: googleToken.refreshToken, expireDate: googleToken.expireDate } })
    });
  });
