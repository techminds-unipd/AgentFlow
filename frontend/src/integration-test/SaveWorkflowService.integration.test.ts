...
describe("SaveWorkflowService - integration", () => {
    ...
    beforeAll(async () => {
        ...
        workflow = new WorkflowDTO(
            "workflowName-" + new Date().toISOString().replace(/[-:.]/g, ""),
            [new NodeDTO(0, { x: 1, y: 1 }, new NodeDataDTO("GMAIL")), new NodeDTO(1, { x: 1, y: 1 }, new NodeDataDTO("GMAIL"))],
            [new EdgeDTO("automate", 0, 1)]
        );
    });

    test("TIF13 - Should return the just saved workflow", async () => {
    ...
    test("TIF14 - Should throw an error with a message if status != 200 is received", async () => {
