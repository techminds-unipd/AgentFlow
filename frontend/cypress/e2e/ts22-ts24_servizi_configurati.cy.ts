describe("Servizi TS22-TS24", () => {
    ...
    it("TS22 - L'utente autenticato può visualizzare quali servizi sono stati configurati", () => {
        ...
        cy.loginByGoogleApi();
        cy.visit("/services");
        cy.get("[data-cy='button-Gmail']").should("have.css", "color", "rgb(241, 102, 16)");
        cy.get("[data-cy='button-GCalendar']").should("have.css", "color", "rgb(241, 102, 16)");
    });

    it("TS23 - L'utente autenticato, visualizzando i blocchi configurati, può visualizzare il nome di ogni blocco", () => {
        cy.loginByGoogleApi();
        cy.visit("/services");
        cy.get("[data-cy='button-Gmail']").should("contain", "Gmail");
        cy.get("[data-cy='button-GCalendar']").should("contain", "GCalendar");
        cy.get("[data-cy='button-Pastebin']").should("contain", "Pastebin");
    });
    ...
    it("TS24 - L'utente autenticato, visualizzando i blocchi configurati, può visualizzare un'opzione per ottenere informazioni sul servizio associato al blocco", () => {
        ...
        cy.get("[data-cy='services-button-GCalendar-info']").should("be.visible");
