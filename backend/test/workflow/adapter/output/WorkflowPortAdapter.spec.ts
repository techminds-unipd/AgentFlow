...
describe("WorkflowPortAdapter", () => {
    ...
    let workflowRepositoryMock: { getWorkflowByName: jest.Mock; };
    ...
    const createTestingModule = async () => {
        const module: TestingModule = await Test.createTestingModule({
            providers: [WorkflowPortAdapter, { provide: WorkflowRepository, useValue: workflowRepositoryMock }]
        }).compile();
        workflowPortAdapter = module.get<WorkflowPortAdapter>(WorkflowPortAdapter);
    };

    beforeEach(async () => {
        workflowRepositoryMock = { getWorkflowByName: jest.fn() };
